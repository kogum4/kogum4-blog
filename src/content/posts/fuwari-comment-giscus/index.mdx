---
title: Fuwariブログにgiscusのコメント機能を追加する方法
...
Astro Fuwariテンプレートで作成したブログに、**giscus**（GitHub Discussionsを利用するコメントシステム）を追加する方法を紹介します。以下の手順で、簡単にコメント欄を実装できます。
...
以上で、Fuwari公式テンプレートから作成したリポジトリに、giscusコメント機能を手動で追加する方法の解説は完了です。古い`comments`ブランチを使わず、必要なコンポーネントを自作して組み込むことで、シンプルかつ柔軟にコメント欄を実装できます。ぜひお試しください！